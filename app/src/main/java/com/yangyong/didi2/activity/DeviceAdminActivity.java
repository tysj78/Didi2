...
//        SystemClock.sleep(100);
        ...
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.KITKAT_WATCH) {
            ...
            UsageStatsManager mUsageStatsManager = (UsageStatsManager) context.getSystemService(Context.USAGE_STATS_SERVICE);
            long time = System.currentTimeMillis();
//            Log.e(TAG, "getCurrentPkgName: "+ time);INTERVAL_DAILY INTERVAL_BEST
...
//                Log.d("sjx", "package == " + event.getPackageName() + ",  type == " + event.getEventType());
                ...
                if (event.getEventType() == UsageEvents.Event.MOVE_TO_FOREGROUND) {
                    //获取这个前台事件的packageName.
                    String tname = event.getPackageName();
                    //Log.d("sjx","topPackageName == " + topPackageName);
                    Log.e(TAG, "前台应用: " + tname);
                }
            }


            List<UsageStats> px = px(stats);
            Log.e(TAG, "stats.size: " + px.size());
            if (px != null) {
                SortedMap<Long, UsageStats> mySortedMap = new TreeMap<Long, UsageStats>();
                for (UsageStats usageStats : px) {
                    Log.e(TAG, "stats_key: " + usageStats.getLastTimeUsed() + "===value:" + usageStats.getPackageName());
                    mySortedMap.put(usageStats.getLastTimeUsed(), usageStats);
                }
                Log.e(TAG, "mySortedMap.size: " + mySortedMap.size());
                for (SortedMap.Entry<Long, UsageStats> entry : mySortedMap.entrySet()) {
//                    Log.e(TAG, "mySortedMapKey = " + entry.getKey() + ", Value = " + entry.getValue().getPackageName());
                }
