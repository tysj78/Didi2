...
//                Log.i(TAG, " EventUtils-getEventList()  " + e.getTimeStamp() + "   event:" + e.getClassName() + "   type = " + e.getEventType());
    ...
    @SuppressWarnings("ResourceType")
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static ArrayList getUsageList(Context context, String pkg, long startTime, long endTime) {
        ...
        for (Map.Entry<String, UsageStats> entry : map.entrySet()) {
            UsageStats stats = (UsageStats) entry.getValue();
            if (stats.getPackageName().equals(pkg) && stats.getTotalTimeInForeground() > 0) {
                list.add(stats);
