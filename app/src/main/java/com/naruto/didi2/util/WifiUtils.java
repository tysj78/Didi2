...
//		}
    ...
    /**
     * 取出附近存在的mac，由于路由器创建的wifi mac地址会存在多个
     *
     * @param scanResults
     * @param macList
     */
    public List<WifiInfos> checkWifi(List<ScanResult> scanResults, List<WifiInfos> macList) {
        List<WifiInfos> existMac = new ArrayList<>();
        for (WifiInfos list : macList) {
            String lowerCase = list.getBssid().toLowerCase();
            String substring = lowerCase.substring(0, 8);
            Log.e(TAG, "substring截取: " + substring);
            for (ScanResult result : scanResults) {
                if (result.BSSID.startsWith(list.getBssid())) {
                    list.setSsid(result.SSID);
                    list.setLevel(result.level);
                    existMac.add(list);
                    break;
                }
            }
        ...
        //输出附近存在的wifi
        for (WifiInfos result : existMac) {
            Log.e(TAG, "附近的wifi有: " + result.getBssid() + "===信号强度：" + result.getLevel());
        }
        return existMac;
    }

    public List<WifiInfos> bubbleSort(List<WifiInfos> infos) {
        for (int i = 0; i < infos.size(); ++i) {
            boolean flag = false;
            for (int j = 0; j < infos.size() - i - 1; ++j) {
                if (infos.get(j).getLevel() < infos.get(j + 1).getLevel()) {
                    WifiInfos temp = infos.get(j);
...
//        for (WifiInfos w:infos){
//            Log.e(TAG, "bubbleSort测试: "+w.toString());
//        }
        return infos;
    }
