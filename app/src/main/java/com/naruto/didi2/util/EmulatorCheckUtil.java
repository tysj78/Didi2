...
/**
 * Created by yangyong on 2019/12/18
 */
...
public class EmulatorCheckUtil {
    ...
    public boolean checkIsRunningInEmulator(Context context) {
        if (context == null)
...
//                if (callback != null)
        ...
        switch (platformResult.result) {
            case CheckResult.RESULT_MAYBE_EMULATOR:
                ++suspectCount;
                break;
            case CheckResult.RESULT_LIKELY_EMULATOR:
                suspectCount += 2;
                break;
            case CheckResult.RESULT_EMULATOR:
                return true;
    ...
    private CheckResult checkFeaturesByPlatform() {
        ...
        if (tempValue.contains("android")) {
            result = CheckResult.RESULT_EMULATOR;
        } else if (tempValue.contains("gmin")) {
            //兼容新版逍遥模拟器(7.5.2,7.5.3)
            result = CheckResult.RESULT_LIKELY_EMULATOR;
        } else {
            result = CheckResult.RESULT_UNKNOWN;
        }
    ...
    private CheckResult checkFeaturesByBaseBand() {
        ...
        if (null == baseBandVersion)
            return new CheckResult(CheckResult.RESULT_MAYBE_EMULATOR, null);
        int result;
